<script lang="ts">
  ...
  let businessType = $state<MerchantBusinessType>(MerchantBusinessType.CORPORATION);
  ...
  async function searchBusinessEntity() {
    ...
    try {
      const response = await businessEntityApi.searchByBusinessNumber(businessNumber);
  ...
  async function checkBlacklist() {
    ...
    try {
      ...
      blacklistError = err instanceof Error ? err.message : '블랙리스트 조회 중 오류가 발생했습니다.';
    } finally {
  ...
  function validateForm(): boolean {
    ...
    if (isBlacklisted) {
      newErrors.blacklist = '블랙리스트에 등록된 사업자는 가맹점 등록이 불가합니다.';
    }
  ...
  async function handleSubmit() {
    ...
    const requestData: MerchantCreateRequest = {
      ...
      businessNumber: businessType !== MerchantBusinessType.NON_BUSINESS ? businessNumber.replace(/-/g, '') : undefined,
      corporateNumber: businessType === MerchantBusinessType.CORPORATION ? corporateNumber.replace(/-/g, '') : undefined,
    ...
    try {
      ...
      submitResult = {
        success: false,
        message: err instanceof Error ? err.message : '서버와 통신할 수 없습니다.'
      };
    } finally {
      loading = false;
    }
...
<div class="max-w-[960px] mx-auto px-6 pb-12">
  <div class="mb-8 pb-6 border-b-2 border-foreground">
    <h1 class="text-3xl font-extrabold text-foreground tracking-tight">가맹점 등록</h1>
  ...
  <Card class="shadow-lg">
      ...
      <CardContent class="py-16 px-8">
        <div class="flex flex-col items-center justify-center text-center">
          ...
          <div class="flex gap-4">
            {#if submitResult.success}
              <Button variant="outline" onclick={resetForm}>새 가맹점 등록</Button>
            {:else}
    ...
    {:else}
        ...
        <div class="space-y-8">
          ...
          <div class="pb-8 border-b border-border">
            ...
            <div class="flex flex-col gap-2">
              <Label for="organizationId">영업점 <span class="text-destructive">*</span></Label>
              <select
                id="organizationId"
                value={organizationId}
                onchange={(e) => organizationId = (e.target as HTMLSelectElement).value}
                disabled={loadingOrgs}
                class="h-11 px-4 pr-8 rounded-md border-[1.5px] border-input bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring focus:border-primary transition-all {errors.organizationId ? 'border-destructive focus:ring-destructive/20' : ''}"
              >
              ...
              {#if errors.organizationId}<span class="text-xs text-destructive">{errors.organizationId}</span>{/if}
            </div>
          </div>
          ...
          {#if businessType !== MerchantBusinessType.NON_BUSINESS}
            <div class="pb-8 border-b border-border">
              ...
              {#if blacklistResult}
                <div class="mb-5 p-4 rounded-lg border {blacklistResult.isBlacklisted ? 'border-destructive bg-destructive/5' : 'border-emerald-200 bg-emerald-50/50'}">
                  <div class="flex items-center gap-3">
                    {#if blacklistResult.isBlacklisted}
                      <Badge variant="destructive">블랙리스트</Badge>
                      <span class="text-sm text-destructive font-medium">{blacklistResult.reason || '블랙리스트에 등록된 사업자입니다.'}</span>
                    {:else}
                      <Badge variant="default" class="bg-emerald-500">정상</Badge>
                      <span class="text-sm text-emerald-700 font-medium">블랙리스트에 등록되지 않은 사업자입니다.</span>
                    {/if}
                  </div>
                </div>
              {/if}
              {#if blacklistError}<span class="text-xs text-destructive mb-5 block">{blacklistError}</span>{/if}
              {#if errors.blacklist}<span class="text-xs text-destructive mb-5 block">{errors.blacklist}</span>{/if}
              ...
              {#if businessSearched && !selectedBusinessEntity && !businessSearching}
                <div class="mb-5 p-4 rounded-lg border-2 border-dashed border-amber-300 bg-amber-50/50">
                  <div class="flex items-center gap-2 text-amber-700">
                    ...
                    <span class="text-sm font-medium">등록된 사업자가 없습니다. 직접 입력해주세요.</span>
                  </div>
                </div>
              {/if}
              ...
              {#if businessType === MerchantBusinessType.CORPORATION}
                <div class="flex flex-col gap-2 mb-5">
                  <Label for="corporateNumber">법인등록번호 <span class="text-destructive">*</span></Label>
                  <input
                    id="corporateNumber"
                    type="text"
                    ...
                    maxlength="14"
                    disabled={!!selectedBusinessEntity}
                    class="h-11 px-4 rounded-md border-[1.5px] border-input bg-background text-sm font-mono focus:outline-none focus:ring-2 focus:ring-ring focus:border-primary transition-all {errors.corporateNumber ? 'border-destructive focus:ring-destructive/20' : ''} {selectedBusinessEntity ? 'bg-muted cursor-not-allowed' : ''}"
                  />
                  {#if errors.corporateNumber}<span class="text-xs text-destructive">{errors.corporateNumber}</span>{/if}
                </div>
              {/if}
            </div>
          {/if}
          ...
          <div class="pb-8 border-b border-border">
            ...
            <div class="grid grid-cols-2 gap-5 mb-5">
              ...
              <div class="flex flex-col gap-2">
                <Label for="name">상호 <span class="text-destructive">*</span></Label>
                <input
                  id="name"
                  type="text"
                  value={name}
                  ...
                  disabled={!!selectedBusinessEntity}
                  class="h-11 px-4 rounded-md border-[1.5px] border-input bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring focus:border-primary transition-all {errors.name ? 'border-destructive focus:ring-destructive/20' : ''} {selectedBusinessEntity ? 'bg-muted cursor-not-allowed' : ''}"
                />
            ...
            <div class="grid grid-cols-2 gap-5 mb-5">
              <div class="flex flex-col gap-2">
                <Label for="representative">대표자 {#if businessType === MerchantBusinessType.NON_BUSINESS}<span class="text-destructive">*</span>{/if}</Label>
                <input
                  id="representative"
                  type="text"
                  value={representative}
                  oninput={(e) => representative = (e.target as HTMLInputElement).value}
                  placeholder="대표자명"
                  disabled={!!selectedBusinessEntity}
                ...
                />
                {#if errors.representative}<span class="text-xs text-destructive">{errors.representative}</span>{/if}
              </div>
              <div class="flex flex-col gap-2">
                <Label for="address">주소 {#if businessType === MerchantBusinessType.NON_BUSINESS}<span class="text-destructive">*</span>{/if}</Label>
                <input
                  id="address"
                  type="text"
                  value={address}
                  ...
                  placeholder="사업장 주소"
                  disabled={!!selectedBusinessEntity}
                  class="h-11 px-4 rounded-md border-[1.5px] border-input bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring focus:border-primary transition-all {errors.address ? 'border-destructive focus:ring-destructive/20' : ''} {selectedBusinessEntity ? 'bg-muted cursor-not-allowed' : ''}"
                />
                {#if errors.address}<span class="text-xs text-destructive">{errors.address}</span>{/if}
              </div>
            </div>
          </div>
            ...
            <div class="grid grid-cols-3 gap-5">
              <div class="flex flex-col gap-2">
                ...
                <input
                  ...
                  type="text"
                  value={contactName}
                  oninput={(e) => contactName = (e.target as HTMLInputElement).value}
                  placeholder="담당자명"
              ...
              <div class="flex flex-col gap-2">
                ...
                <input
                  ...
                  type="tel"
                  value={contactPhone}
                  oninput={(e) => contactPhone = (e.target as HTMLInputElement).value}
              ...
              <div class="flex flex-col gap-2">
                ...
                <input
                  ...
                  type="email"
                  value={contactEmail}
                  oninput={(e) => contactEmail = (e.target as HTMLInputElement).value}
                  placeholder="example@email.com"
                  class="h-11 px-4 rounded-md border-[1.5px] border-input bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring focus:border-primary transition-all {errors.contactEmail ? 'border-destructive focus:ring-destructive/20' : ''}"
                />
                {#if errors.contactEmail}<span class="text-xs text-destructive">{errors.contactEmail}</span>{/if}
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="flex justify-end items-center gap-4 px-8 py-6 bg-gradient-to-b from-muted/50 to-muted border-t border-border">
        <Button variant="outline" onclick={resetForm} disabled={loading}>
          취소
        </Button>
        <Button onclick={handleSubmit} disabled={loading || isBlacklisted} class="bg-primary hover:bg-primary/90">
          {#if loading}
