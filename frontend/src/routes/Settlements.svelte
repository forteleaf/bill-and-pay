<script lang="ts">
  ...
  async function handleResettle() {
    ...
    try {
      const response = await settlementApi.resettleByEventId(resettleTargetEventId);

      if (response.success) {
        toast.success(`재정산 완료: ${response.data?.length || 0}건의 새 정산이 생성되었습니다.`);
        loadSettlements();
        loadSummary();
      } else {
      ...
      toast.error(err instanceof Error ? err.message : '재정산 중 오류가 발생했습니다.');
  ...
  <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
    <Card class="border-l-4 border-l-yellow-500">
      <CardHeader class="pb-2">
        <CardTitle class="text-sm font-medium text-muted-foreground">대기</CardTitle>
      </CardHeader>
      <CardContent>
    ...
    <Card class="border-l-4 border-l-blue-500">
      <CardHeader class="pb-2">
        <CardTitle class="text-sm font-medium text-muted-foreground">승인</CardTitle>
      </CardHeader>
      <CardContent>
    ...
    <Card class="border-l-4 border-l-green-500">
      <CardHeader class="pb-2">
        <CardTitle class="text-sm font-medium text-muted-foreground">지급 완료</CardTitle>
      </CardHeader>
      <CardContent>
    ...
    <CardContent class="pt-6">
      <div class="flex flex-wrap gap-4 items-end">
        <div class="flex flex-col gap-1.5">
          <Label>상태</Label>
          <Select.Root type="single" value={statusFilter} onValueChange={(v) => { statusFilter = v || 'ALL'; loadSettlements(); }}>
    ...
    <div class="flex justify-center py-12">
      <Alert variant="destructive" class="max-w-lg">
        ...
        <AlertDescription class="flex flex-col gap-3">
          <span>{error}</span>
          <Button variant="outline" size="sm" onclick={loadSettlements} class="self-start">
            다시 시도
          </Button>
        </AlertDescription>
      </Alert>
              ...
              <TableCell>
                {#if isResettleable(settlement.status)}
                  <Button
                    ...
                    size="sm"
                    disabled={resettling}
                    onclick={() => openResettleModal(settlement.transactionEventId)}
                  >
                    재정산
                  </Button>
                {/if}
...
<ConfirmModal
  ...
  onCancel={() => { showResettleModal = false; resettleTargetEventId = null; }}
/>
