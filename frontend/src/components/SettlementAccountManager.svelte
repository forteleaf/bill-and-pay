<script lang="ts">
  ...
  async function loadAccounts() {
    ...
    try {
      const response = await settlementAccountApi.listByEntity(entityType, entityId);
      if (response.success && response.data) {
        accounts = response.data.content || [];
      } else {
        error = response.error?.message || '정산계좌 목록을 불러올 수 없습니다.';
      }
    } catch (err) {
  ...
  async function handleSave() {
    ...
    try {
      if (dialogMode === 'add') {
        ...
        const response = await settlementAccountApi.update(editingAccount.id, request);
        if (response.success) {
      ...
      error = dialogMode === 'add' ? '계좌 등록에 실패했습니다.' : '계좌 수정에 실패했습니다.';
    } finally {
      saving = false;
    }
  }
  ...
  function getStatusVariant(status: SettlementAccountStatus): 'default' | 'secondary' | 'destructive' | 'outline' {
    switch (status) {
  ...
  <CardContent>
    ...
    {:else if error}
      <div class="flex flex-col items-center justify-center py-8 gap-3 text-destructive">
        ...
        <Button variant="outline" size="sm" onclick={loadAccounts}>다시 시도</Button>
      </div>
                ...
                <div class="min-w-0 flex-1">
                  <div class="flex items-center gap-2 flex-wrap">
                    <span class="font-semibold text-foreground">
                      {account.bankName || KOREAN_BANK_CODES[account.bankCode] || account.bankCode}
                    </span>
                    ...
                    {/if}
                    <Badge variant={getStatusVariant(account.status)} class="text-xs">
                  ...
                  {#if account.memo}
                    <p class="mt-2 text-xs text-muted-foreground bg-muted/50 rounded px-2 py-1 inline-block">
                      {account.memo}
                    </p>
                  {/if}
                </div>
  ...
  <DialogPrimitive.Portal>
    ...
    >
      <div class="flex items-center justify-between px-6 py-4 border-b border-border">
        <div>
          ...
          <DialogPrimitive.Description class="text-sm text-muted-foreground mt-0.5">
            {dialogMode === 'add' ? '새로운 정산계좌 정보를 입력하세요' : '계좌 정보를 수정합니다'}
      ...
      <div class="px-6 py-5 space-y-5">
        <!-- Bank Selection -->
        <div class="space-y-2">
          <Label for="bank-select" class="text-sm font-medium">은행 <span class="text-destructive">*</span></Label>
          <select
            id="bank-select"
        ...
        <!-- Account Number -->
        <div class="space-y-2">
          <Label for="account-number" class="text-sm font-medium">계좌번호 <span class="text-destructive">*</span></Label>
          <Input
            id="account-number"
            type="text"
            placeholder="'-' 없이 숫자만 입력"
            value={formAccountNumber}
            oninput={(e) => formAccountNumber = e.currentTarget.value}
            class="font-mono"
          />
        </div>
        ...
        <!-- Account Holder -->
        <div class="space-y-2">
          <Label for="account-holder" class="text-sm font-medium">예금주 <span class="text-destructive">*</span></Label>
          <Input
            id="account-holder"
            type="text"
        ...
        <label class="flex items-center gap-3 p-3 rounded-lg border border-border hover:bg-muted/50 cursor-pointer transition-colors">
          ...
          />
          <div>
            <span class="font-medium text-sm text-foreground">주계좌로 설정</span>
            <p class="text-xs text-muted-foreground mt-0.5">정산금이 이 계좌로 입금됩니다</p>
          </div>
        </label>
      </div>
      ...
      <div class="flex justify-end gap-3 px-6 py-4 border-t border-border bg-muted/30">
        ...
        <Button 
          onclick={handleSave} 
          disabled={saving || !formBankCode || !formAccountNumber || !formAccountHolder}
        >
          {#if saving}
    ...
    >
      <div class="p-6">
        ...
        <DialogPrimitive.Description class="text-sm text-muted-foreground text-center mt-2">
          {#if accountToDelete}
            <span class="font-medium text-foreground">{accountToDelete.bankName || KOREAN_BANK_CODES[accountToDelete.bankCode]}</span>
            <span class="font-mono"> ({accountToDelete.maskedAccountNumber})</span>
            <br/>계좌를 삭제하시겠습니까?
          {/if}
        </DialogPrimitive.Description>
