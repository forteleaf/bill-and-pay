...
<script lang="ts">
  ...
  const ORG_TYPE_COLORS: Record<string, { bg: string; border: string; text: string; gradient: string; ring: string }> = {
    DISTRIBUTOR: { 
      bg: 'bg-indigo-500/10', 
  ...
  let colors = $derived(ORG_TYPE_COLORS[data.orgType] || ORG_TYPE_COLORS.DISTRIBUTOR);
  let canAddChild = $derived(ORG_TYPE_ORDER.indexOf(data.orgType) < ORG_TYPE_ORDER.length - 1);
  ...
  >
    ...
    <div class="px-4 py-3">
      <div class="flex items-center gap-2 mb-2">
        <div class="w-8 h-8 rounded-lg bg-gradient-to-br {colors.gradient} flex items-center justify-center text-white text-sm font-bold shadow-sm shrink-0">
          {ORG_TYPE_LABELS[data.orgType]?.charAt(0) || 'O'}
        </div>
        <div class="flex-1 min-w-0">
          <div class="font-semibold text-sm text-foreground truncate leading-tight">
            {data.name}
          </div>
      ...
      <div class="flex items-center justify-between gap-2">
        <span class="text-xs {colors.text} font-medium px-2 py-0.5 rounded-md {colors.bg}">
          {ORG_TYPE_LABELS[data.orgType]}
        </span>
        {#if data.status === 'ACTIVE'}
          <Badge variant="default" class="text-xs px-1.5 py-0 h-5 bg-emerald-500 hover:bg-emerald-500">
            {STATUS_LABELS[data.status]}
          </Badge>
        {:else if data.status === 'SUSPENDED'}
          <Badge variant="secondary" class="text-xs px-1.5 py-0 h-5 bg-amber-500 text-white hover:bg-amber-500">
            {STATUS_LABELS[data.status]}
          </Badge>
        {:else}
