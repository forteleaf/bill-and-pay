  ...
  async function loadTransaction(transactionId: string) {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        transactionError = response.error?.message || '원거래 정보를 불러올 수 없습니다.';
      }
    } catch (err) {
      transactionError = err instanceof Error ? err.message : '원거래 정보를 불러올 수 없습니다.';
    } finally {
          ...
          <div>
            ...
            <div class="grid grid-cols-2 gap-4">
              <div class="space-y-1">
                ...
                <Badge variant="outline" class="font-medium">
                  {ENTITY_TYPE_LABELS[settlement.entityType] || settlement.entityType}
                </Badge>
              </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div class="space-y-1">
                ...
                <p class="text-lg font-semibold">{formatCurrency(settlement.amount)}</p>
              </div>
              
              <div class="space-y-1">
                <p class="text-xs text-muted-foreground">수수료율</p>
                <p class="text-lg font-medium text-muted-foreground">{formatPercent(settlement.feeRate)}</p>
              </div>
              
              <div class="space-y-1">
                <p class="text-xs text-muted-foreground">수수료</p>
                <p class="text-lg font-medium text-muted-foreground">{formatCurrency(settlement.feeAmount)}</p>
              </div>
              
              <div class="space-y-1">
                <p class="text-xs text-muted-foreground">정산금액</p>
                <p class="text-lg font-bold text-primary">{formatCurrency(settlement.netAmount)}</p>
              </div>
            </div>
              ...
              <div class="bg-muted/50 rounded-lg p-4 space-y-4">
                <div class="grid grid-cols-2 gap-4">
                  <div class="space-y-1">
                    ...
                    <p class="font-mono text-sm font-medium">{transaction.transactionId}</p>
                  </div>
                  
                  <div class="space-y-1">
                    <p class="text-xs text-muted-foreground">거래상태</p>
                    <Badge variant={getStatusVariant(transaction.status)}>
                      {TX_STATUS_LABELS[transaction.status] || transaction.status}
                    </Badge>
                  </div>
                  ...
                  <div class="space-y-1">
                    ...
                    <p class="font-mono text-xs">{transaction.pgTransactionId}</p>
                  </div>
