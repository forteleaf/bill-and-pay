...
- [PRD-03: 원장/정산](PRD-03_ledger.md) - 조직별 정산 규칙
- [PRD-05: DB 스키마](PRD-05_database_schema.md) - organizations 테이블 상세
- [PRD-07: UI 화면 설계](PRD-07_ui_screens.md) - 영업점 관리 화면
...
| 계층 | 가맹점 관리 | 하위 조직 생성 | 정산 조회 | PG 설정 | 알림 수신 |
|------|:----------:|:------------:|:--------:|:------:|:--------:|
| 총판 | 전체 | 전체 | 전체 | ✅ | 시스템 알림 |
| 대리점 | 하위 전체 | 에이전시~벤더 | 하위 전체 | ❌ | 결제/정산 알림 |
| 에이전시 | 하위만 | 딜러~벤더 | 하위만 | ❌ | ❌ |
| 딜러 | 하위만 | 셀러~벤더 | 하위만 | ❌ | ❌ |
| 셀러 | 하위만 | 벤더 | 하위만 | ❌ | ❌ |
| 벤더 | 하위만 | ❌ | 하위만 | ❌ | ❌ |
...
├─ 신규 거래 정산 중단
├─ 기존 정산 내역은 보존
├─ 진행 중인 세션 강제 만료 (JWT 블랙리스트)
└─ 하위 조직 상태도 일괄 SUSPENDED로 변경
```

**상태 변경 시나리오**:

| 시나리오 | 현재 상태 | 변경 상태 | 영향 범위 |
|---------|---------|---------|---------|
| 일시 정지 | ACTIVE | SUSPENDED | 해당 조직 + 모든 하위 조직 |
| 정지 해제 | SUSPENDED | ACTIVE | 해당 조직만 (하위는 개별 해제) |
| 계약 해지 | ACTIVE/SUSPENDED | TERMINATED | 해당 조직 + 모든 하위 조직 (비가역적) |
...
| v2.1 | 2026-02-07 | 계층별 책임 범위 추가, 상태 변경 영향도 보강, 상호 참조 링크 추가 |
