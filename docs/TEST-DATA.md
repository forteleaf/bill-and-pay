...
개발 및 테스트 환경에서 사용할 시드 데이터입니다.

## 데이터 현황

| 테이블 | 건수 | 설명 |
|--------|------|------|
| organizations | 10 | 5단계 계층 구조 (총판→대리점→딜러→셀러→벤더) |
...
| busan_admin | busan@agency.com | AGENCY_ADMIN | password123 |
| dealer1 | gangnam@dealer.com | DEALER | password123 |
| seller1 | yeoksam@seller.com | SELLER | password123 |
| merchant1 | coffee@merchant.com | MERCHANT | password123 |

> 모든 테스트 계정의 비밀번호는 `password123` 입니다.

## 참고사항

- `public.users`의 testuser 비밀번호도 `password123`으로 업데이트됨
