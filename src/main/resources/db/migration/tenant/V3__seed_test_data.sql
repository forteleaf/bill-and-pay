...
-- 셀러 사업자
('01960000-0000-7000-0000-000000000006', 'INDIVIDUAL', '6789012345', NULL, '역삼셀러', '이판매', '2023-01-05', '서울시 강남구 역삼로 50', '서울시 강남구 역삼로 50', '서비스업', '단말기판매', '0256789012', NULL, '01067890123', 'yeoksam@seller.com'),
('01960000-0000-7000-0000-000000000007', 'INDIVIDUAL', '7890123456', NULL, '삼성셀러', '김판매', '2023-02-10', '서울시 강남구 삼성로 60', '서울시 강남구 삼성로 60', '서비스업', '단말기판매', '0267890123', NULL, '01078901234', 'samsung@seller.com'),
-- 벤더 사업자
...
-- Level 4: SELLER (셀러)
INSERT INTO organizations (id, org_code, name, org_type, path, parent_id, level, status, business_entity_id) VALUES
('01960000-0000-7000-0001-000000000006', 'SELL001', '역삼셀러', 'SELLER', 'dist_001.agcy_001.deal_001.sell_001', '01960000-0000-7000-0001-000000000004', 4, 'ACTIVE', '01960000-0000-7000-0000-000000000006'),
('01960000-0000-7000-0001-000000000007', 'SELL002', '삼성셀러', 'SELLER', 'dist_001.agcy_001.deal_001.sell_002', '01960000-0000-7000-0001-000000000004', 4, 'ACTIVE', '01960000-0000-7000-0000-000000000007');
...
BEGIN
  ...
  IF NOT v_exists THEN
    ...
    VALUES (v_txn_id_1, 'TXN' || to_char(CURRENT_DATE, 'YYYYMMDD') || '001', '01960000-0000-7000-0003-000000000001', '01960000-0000-7000-0004-000000000001', 1, 'dist_001.agcy_001.deal_001.sell_001.vend_001', v_credit_method_id, v_kb_card_id, 15000, 'APPROVED', 'PG_TXN_001', 'APR001', v_now - INTERVAL '2 hours', 'CAT001', v_now - INTERVAL '2 hours');
    ...
    VALUES (v_event_id_1, 'APPROVAL', 1, v_txn_id_1, '01960000-0000-7000-0003-000000000001', '01960000-0000-7000-0004-000000000001', 1, 'dist_001.agcy_001.deal_001.sell_001.vend_001', v_credit_method_id, v_kb_card_id, 15000, 'PENDING', 'APPROVED', 'PG_TXN_001', 'APR001', 'CAT001', v_now - INTERVAL '2 hours', v_now - INTERVAL '2 hours');
  END IF;

  SELECT EXISTS(SELECT 1 FROM transactions WHERE id = v_txn_id_2) INTO v_exists;
  IF NOT v_exists THEN
    INSERT INTO transactions (id, transaction_id, merchant_id, merchant_pg_mapping_id, pg_connection_id, org_path, payment_method_id, card_company_id, amount, status, pg_transaction_id, approval_number, approved_at, cat_id, created_at)
    VALUES (v_txn_id_2, 'TXN' || to_char(CURRENT_DATE, 'YYYYMMDD') || '002', '01960000-0000-7000-0003-000000000002', '01960000-0000-7000-0004-000000000002', 1, 'dist_001.agcy_001.deal_001.sell_001.vend_002', v_debit_method_id, v_sh_card_id, 8500, 'APPROVED', 'PG_TXN_002', 'APR002', v_now - INTERVAL '1 hour', 'CAT002', v_now - INTERVAL '1 hour');

    INSERT INTO transaction_events (id, event_type, event_sequence, transaction_id, merchant_id, merchant_pg_mapping_id, pg_connection_id, org_path, payment_method_id, card_company_id, amount, previous_status, new_status, pg_transaction_id, approval_number, cat_id, occurred_at, created_at)
    VALUES (v_event_id_2, 'APPROVAL', 1, v_txn_id_2, '01960000-0000-7000-0003-000000000002', '01960000-0000-7000-0004-000000000002', 1, 'dist_001.agcy_001.deal_001.sell_001.vend_002', v_debit_method_id, v_sh_card_id, 8500, 'PENDING', 'APPROVED', 'PG_TXN_002', 'APR002', 'CAT002', v_now - INTERVAL '1 hour', v_now - INTERVAL '1 hour');
  END IF;

  SELECT EXISTS(SELECT 1 FROM transactions WHERE id = v_txn_id_3) INTO v_exists;
  IF NOT v_exists THEN
    INSERT INTO transactions (id, transaction_id, merchant_id, merchant_pg_mapping_id, pg_connection_id, org_path, payment_method_id, card_company_id, amount, status, pg_transaction_id, approval_number, approved_at, cat_id, created_at)
    VALUES (v_txn_id_3, 'TXN' || to_char(CURRENT_DATE, 'YYYYMMDD') || '003', '01960000-0000-7000-0003-000000000003', '01960000-0000-7000-0004-000000000003', 1, 'dist_001.agcy_001.deal_001.sell_002.vend_003', v_credit_method_id, v_ss_card_id, 125000, 'APPROVED', 'PG_TXN_003', 'APR003', v_now - INTERVAL '30 minutes', 'CAT003', v_now - INTERVAL '30 minutes');

    INSERT INTO transaction_events (id, event_type, event_sequence, transaction_id, merchant_id, merchant_pg_mapping_id, pg_connection_id, org_path, payment_method_id, card_company_id, amount, previous_status, new_status, pg_transaction_id, approval_number, cat_id, occurred_at, created_at)
    VALUES (v_event_id_3, 'APPROVAL', 1, v_txn_id_3, '01960000-0000-7000-0003-000000000003', '01960000-0000-7000-0004-000000000003', 1, 'dist_001.agcy_001.deal_001.sell_002.vend_003', v_credit_method_id, v_ss_card_id, 125000, 'PENDING', 'APPROVED', 'PG_TXN_003', 'APR003', 'CAT003', v_now - INTERVAL '30 minutes', v_now - INTERVAL '30 minutes');
  END IF;
END $$;

INSERT INTO settlements (id, transaction_event_id, transaction_id, merchant_id, org_path, entity_id, entity_type, entity_path, entry_type, amount, fee_amount, net_amount, fee_rate, status)
SELECT * FROM (VALUES
...
) AS v(id, transaction_event_id, transaction_id, merchant_id, org_path, entity_id, entity_type, entity_path, entry_type, amount, fee_amount, net_amount, fee_rate, status)
